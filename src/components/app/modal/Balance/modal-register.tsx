...
interface ModalRegisterBalanceProps {
  isOpen: boolean;
...
export default function ModalRegisterBalance({
  isOpen,
  setIsOpen,
}: ModalRegisterBalanceProps) {
  ...
  async function onSubmit({
    ...
    const dataBalance = {
      ...
      name,
      remainingValue: parseFloat(initialValueFormat.toFixed(2)),
      userId: userCookie?.uid ?? "",
    };

    await createBalance(dataBalance);
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Criar saldo</DialogTitle>
            <DialogDescription>
