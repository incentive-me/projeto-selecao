...
export default function ModalDeletePayment({
  isOpen,
  setIsOpen,
}: ModalDeletePaymentProps) {
  const { deletePaymentById, paymentForId } = usePayment();
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="flex gap-2">
            <Trash2Icon className="h-6 w-6 text-red-600" /> Excluir pagamento ?
          </AlertDialogTitle>
          <AlertDialogDescription>
            Se excluir este pagamento, esta ação não poderá ser revertida. Tem
